...
//    qDebug()<<"size:"<<bySize;
//    qDebug()<<"inf:"<<byInf;
//    qDebug()<<"group:"<<byGroup;
//    qDebug()<<"value:"<<
//              DeviceProtocol::GDDValue(GID,byType,bySize,byNum);
...
void SettingHandler::ReadSettingSetting()
{
    ...
    m_lstReadCmd.clear();
    QVariantMap map;
    QList<int> nos = m_device->GetGroupNo(enumGroupTypeSettingArea);
    if(nos.isEmpty()){
        SettingResult(false,
                      "103规约：没有定值区组",
                      true);
    ...
    map["readType"] = KOD_RANGE;
    m_lstReadCmd.append(map);

    foreach (int no, m_device->GetGroupNo(enumGroupTypeSetting)) {
        AddGroupToCommand(no);
    }
