...
bool Pro103AppPrivate::Start()
{
    ...
    QVariantList list;
    QFile file("Device_Info\\Device\\device.ini");
    if(file.exists()){
        file.remove();
    }
    foreach (DbObjectPtr dev, ScadaObject::GetInstance()
        ...
        int num_old=device->value("NUM", 0).toInt();
        int num_new=num_old+1;
        device->setValue("NUM", num_new);
        QDateTime current_date_time =QDateTime::currentDateTime();
        QString current_date =current_date_time.toString("yyyy.MM.dd hh:mm:ss");
        device->setValue("CREATE_TIME", current_date);
        device->endGroup();
